class UpdateLastSeenJob < ApplicationJob
  ...
  def perform(conversation_id, user, agent_last_seen_at)
    agent_viewed(conversation_id, user) if conversation.account.feature_enabled?('agent_conversation_viewed')

    read_message(conversation_id, agent_last_seen_at) if conversation.account.feature_enabled?('read_message')
  end

  private

  def read_message(conversation_id, agent_last_seen_at)
  ...
  def agent_viewed(conversation_id, user)
    conversation = Conversation.find(conversation_id)
